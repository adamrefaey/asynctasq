...
def add_driver_args(parser: argparse.ArgumentParser, default_driver: str | None = None) -> None:
    ...
    parser.add_argument(
        ...
        help="Queue driver to use (default: from async_task_q_DRIVER env var or 'redis')",
    )
    ...
    redis_group.add_argument(
        "--redis-url",
        type=str,
        help="Redis connection URL (default: from async_task_q_REDIS_URL env var or 'redis://localhost:6379')",
    )
    redis_group.add_argument(
        "--redis-password",
        type=str,
        help="Redis password (default: from async_task_q_REDIS_PASSWORD env var)",
    )
    redis_group.add_argument(
        "--redis-db",
        type=int,
        help="Redis database number (default: from async_task_q_REDIS_DB env var or 0)",
    )
    redis_group.add_argument(
        "--redis-max-connections",
        type=int,
        help="Redis max connections (default: from async_task_q_REDIS_MAX_CONNECTIONS env var or 10)",
    )
    ...
    sqs_group.add_argument(
        "--sqs-region",
        type=str,
        help="AWS SQS region (default: from async_task_q_SQS_REGION env var or 'us-east-1')",
    )
    sqs_group.add_argument(
        "--sqs-queue-url-prefix",
        type=str,
        help="SQS queue URL prefix (default: from async_task_q_SQS_QUEUE_PREFIX env var)",
    )
    sqs_group.add_argument(
        "--aws-access-key-id",
        type=str,
    ...
    postgres_group.add_argument(
        "--postgres-dsn",
        type=str,
        help="PostgreSQL connection DSN (default: from async_task_q_POSTGRES_DSN env var)",
    )
    postgres_group.add_argument(
        "--postgres-queue-table",
        type=str,
        help="PostgreSQL queue table name (default: from async_task_q_POSTGRES_QUEUE_TABLE env var or 'task_queue')",
    )
    postgres_group.add_argument(
        "--postgres-dead-letter-table",
        type=str,
        help="PostgreSQL dead letter table name (default: from async_task_q_POSTGRES_DEAD_LETTER_TABLE env var or 'dead_letter_queue')",
    )
    ...
    mysql_group.add_argument(
        "--mysql-dsn",
        type=str,
        help="MySQL connection DSN (default: from async_task_q_MYSQL_DSN env var)",
    )
    mysql_group.add_argument(
        "--mysql-queue-table",
        type=str,
        help="MySQL queue table name (default: from async_task_q_MYSQL_QUEUE_TABLE env var or 'task_queue')",
    )
    mysql_group.add_argument(
        "--mysql-dead-letter-table",
        type=str,
        help="MySQL dead letter table name (default: from async_task_q_MYSQL_DEAD_LETTER_TABLE env var or 'dead_letter_queue')",
    )
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Async Task Q - Task queue system for Python",

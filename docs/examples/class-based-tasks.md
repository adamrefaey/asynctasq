...
This guide provides concrete, ready-to-use code examples demonstrating all scenarios, options, and capabilities of class-based tasks in Async Task.
...
Async Task supports both async and synchronous class-based tasks. The framework automatically handles the execution differences:
...
```python
from async_task.core.task import Task, SyncTask
import asyncio
import time
...
Class-based tasks in Async Task provide a powerful, flexible way to create reusable, testable background tasks with complete control over execution lifecycle.

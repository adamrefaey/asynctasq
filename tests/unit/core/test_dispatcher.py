...
class TestGetDispatcher:
    ...
    def test_get_dispatcher_with_none_uses_default_config(self) -> None:
        ...
        with (
            patch("async_task.core.dispatcher.get_global_config") as mock_get_config,
            patch("async_task.core.dispatcher.DriverFactory.create_from_config") as mock_create,
            patch("async_task.core.dispatcher.Dispatcher") as mock_dispatcher_class,
        ):
            mock_config = MagicMock(spec=Config)
            mock_get_config.return_value = mock_config
            mock_driver = MagicMock(spec=BaseDriver)
    ...
    def test_get_dispatcher_with_base_driver_instance(self) -> None:
        # Arrange
        mock_driver = MagicMock(spec=BaseDriver)
        with patch("async_task.core.dispatcher.Dispatcher") as mock_dispatcher_class:
            mock_dispatcher = MagicMock(spec=Dispatcher)
            mock_dispatcher_class.return_value = mock_dispatcher
    ...
    def test_get_dispatcher_handles_config_none_creates_from_env(self) -> None:
        ...
        with (
            patch("async_task.core.dispatcher.get_global_config") as mock_get_config,
            patch("async_task.core.dispatcher.Config.from_env") as mock_from_env,
            patch("async_task.core.dispatcher.DriverFactory.create_from_config") as mock_create,
            patch("async_task.core.dispatcher.Dispatcher") as mock_dispatcher_class,
        ):
